...
theorem isotropicGaussianPDF_one_eq_prefactor_mul_kinetic
    (p : Momentum ι) :
    McmcLean.Kernel.isotropicGaussianPDF (ι := ι) 1 p =
      standardMomentumPrefactor (ι := ι) * kineticBoltzmannWeight p := by
  unfold McmcLean.Kernel.isotropicGaussianPDF gaussianPDF gaussianPDFReal
...
theorem standardMomentumMeasure_eq_smul_kinetic :
  ...
  rw [show McmcLean.Kernel.isotropicGaussianPDF (ι := ι) 1 =
