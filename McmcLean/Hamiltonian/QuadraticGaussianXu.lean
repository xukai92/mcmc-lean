    ...
    ∃ (γ : Set.Ioo (0 : NNReal) 1) (ell1 : ENNReal),
      xuTheorem41Lambda0 γ driftCoefficient
        ...
        xuTheorem41PairedRate γ driftCoefficient driftAllowance
          (2 + McmcLean.Kernel.isotropicGaussianFirstNormMoment
            (ι := ι) variance) ell1 < 1 := by
  let growth := 2 + McmcLean.Kernel.isotropicGaussianFirstNormMoment
    (ι := ι) variance
...
theorem XuTheorem41DriftAssumptions.isCompact_pairedSublevel_of_V_eq_standardDistance
    ...
    IsCompact (McmcLean.Kernel.lyapunovSublevel
      (McmcLean.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1)) := by
  simpa only [hV] using
...
theorem XuTheorem41DriftAssumptions.nonempty_pairedSublevel_of_V_eq_standardDistance
    ...
    (McmcLean.Kernel.lyapunovSublevel
      (McmcLean.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1)).Nonempty := by
