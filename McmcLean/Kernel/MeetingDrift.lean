...
theorem IsFaithfulPath.mem_exactMeetingFailureEvent_iff
  ...
  rw [McmcLean.Kernel.mem_exactMeetingFailureEvent_iff, show
...
theorem HasGeometricDrift.stickyCoupling
  ...
  by_cases hx : x ∈ Set.diagonal α
  · rw [McmcLean.Kernel.stickyCoupling, Kernel.piecewise_apply, if_pos hx]
    exact hdiagonal x hx
  · rw [McmcLean.Kernel.stickyCoupling, Kernel.piecewise_apply, if_neg hx]
    exact hdrift.2.2 x
...
theorem HasGeometricDrift.stickyCoupling_pairedAdd
    ...
    (hdrift : HasGeometricDrift coupled (IsCoupling.pairedAdd v) C
      rate allowance) :
    HasGeometricDrift (McmcLean.Kernel.stickyCoupling transition coupled)
