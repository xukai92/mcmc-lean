...
theorem gaussianZigZagCyclePhaseEnvironmentShift_iterate_tail
  ...
  induction eventCount generalizing index with
  | zero => simp
  | succ eventCount ih =>
      ...
      calc
        _ = input.1.2 ((index + 1) + eventCount) := ih (index + 1)
        _ = input.1.2 (index + (eventCount + 1)) := by
          rw [Nat.add_assoc, Nat.add_comm 1 eventCount]
...
theorem gaussianZigZagSqrtHazard_le_phaseRoof_iterate_succ
  ...
  have hadvancedLeft : advanced.1.1.1 ≤ 0 := by
    change ((gaussianZigZagCyclePhaseEnvironmentShift^[eventCount + 1])
      input).1.1.1 ≤ 0
    rw [show eventCount + 1 = eventCount.succ by omega,
      Function.iterate_succ_apply']
    change ((gaussianZigZagCyclePhaseEnvironmentShift^[eventCount])
      input).1.1.2 ≤ 0
    cases eventCount with
  ...
  change Real.sqrt (2 * (input.1.2 eventCount : ℝ)) ≤
    (Real.toNNReal (-advanced.1.1.2 - advanced.1.1.1) : ℝ)
  rw [Real.coe_toNNReal _ hroofNonneg, hadvancedRight]
  unfold gaussianZigZagNegativeRayleighReset
  linarith
...
theorem gaussianZigZagCyclePhaseEnvironment_nonexplosive_ae :
  ...
  have hsqrtSum : shift < ∑ index ∈ Finset.range eventCount,
    ...
    change ENNReal.ofReal shift <
      ∑ index ∈ Finset.range eventCount,
        ENNReal.ofReal (Real.sqrt (2 * (input.1.2 index : ℝ))) at hpartial
    rw [← ENNReal.ofReal_sum_of_nonneg
      (fun index _ => Real.sqrt_nonneg _)] at hpartial
    have hsumPos : 0 < ∑ index ∈ Finset.range eventCount,
        Real.sqrt (2 * (input.1.2 index : ℝ)) := by
      by_contra hnonpos
      have hzero : ENNReal.ofReal (∑ index ∈ Finset.range eventCount,
          Real.sqrt (2 * (input.1.2 index : ℝ))) = 0 :=
        ENNReal.ofReal_eq_zero.mpr (le_of_not_gt hnonpos)
      rw [hzero] at hpartial
      exact (not_lt_of_ge bot_le) hpartial
    exact (ENNReal.ofReal_lt_ofReal_iff hsumPos).mp hpartial
  have hlower : (∑ index ∈ Finset.range eventCount,
    ...
    calc
      _ ≤ ∑ index ∈ Finset.range eventCount,
          gaussianZigZagCyclePhaseEnvironmentRoof
            ((gaussianZigZagCyclePhaseEnvironmentShift^[index + 1]) input) :=
        Finset.sum_le_sum fun index _ =>
          gaussianZigZagSqrtHazard_le_phaseRoof_iterate_succ
            input hnegativeInput.2 index
      _ ≤ _ := le_add_of_nonneg_right (by
        unfold gaussianZigZagCyclePhaseEnvironmentRoof
          gaussianZigZagCycleEnvironmentRoof
        positivity)
...
theorem gaussianZigZagCycleAgeOccupation_map :
  ...
  have hnegative : ∀ᵐ resets ∂resetMeasure,
      resets.1 < 0 ∧ resets.2 < 0 := by
    filter_upwards [
      ...
      (Measure.quasiMeasurePreserving_snd
        ...
        (ν := gaussianZigZagNegativeRayleighMeasure)).ae
          gaussianZigZagNegativeRayleighMeasure_negative_ae]
      with resets hleft hright
    exact ⟨hleft, hright⟩
  filter_upwards [hnegative]
  ...
  have hfiber := congrArg (fun measure : Measure ℝ =>
    ...
    (gaussianZigZagSuspension_ageFiber_map
      (resets, fun _ : ℕ => (0 : NNReal)) hnegative.1 hnegative.2)
  rw [Measure.map_apply (by fun_prop)
    (hevent.preimage measurable_prodMk_left)] at hfiber
  have hfiberSet : MeasurableSet
      ((fun age : ℝ =>
        (resets, fun _ : ℕ => (0 : NNReal)).1.1 + age) ⁻¹'
        Prod.mk resets ⁻¹' event) := by
    exact (measurable_const.add measurable_id)
      (hevent.preimage measurable_prodMk_left)
  rw [Measure.restrict_apply hfiberSet] at hfiber
  have hsection : Prod.mk resets ⁻¹'
      ((fun input : (ℝ × ℝ) × ℝ => (input.1, input.1.1 + input.2)) ⁻¹'
        event ∩ {input | input.2 ∈ Set.Ico 0
          (gaussianZigZagCycleEnvironmentRoof
            (input.1, fun _ : ℕ => (0 : NNReal)) : ℝ)}) =
      ((fun age : ℝ => (resets, resets.1 + age)) ⁻¹' event) ∩
        Set.Ico 0
          (gaussianZigZagCycleEnvironmentRoof
            (resets, fun _ : ℕ => (0 : NNReal)) : ℝ) := by
    ext age
    simp
  rw [hsection]
  have heventPre :
      (fun age : ℝ => (resets, resets.1 + age)) ⁻¹' event =
        (fun age : ℝ => resets.1 + age) ⁻¹'
          (Prod.mk resets ⁻¹' event) := rfl
  rw [heventPre]
  simpa using hfiber
...
theorem gaussianZigZagSuspensionDecode_map_occupation :
  ...
  have hpermuteMeasure : Measure.map permute
    ...
    have hp₅ := MeasurePreserving.symm
      (MeasurableEquiv.prodAssoc :
        (((ℝ × ℝ) × (ℝ × Bool)) × (ℕ → NNReal)) ≃ᵐ
          ((ℝ × ℝ) × ((ℝ × Bool) × (ℕ → NNReal))))
      (measurePreserving_prodAssoc resetMeasure
        (volume.prod velocityMeasure) hazardMeasure)
  ...
  have hageMap : Measurable (fun input : (ℝ × ℝ) × ℝ =>
      (input.1, input.1.1 + input.2)) := by fun_prop
  rw [htranslateFactor,
    ← Measure.map_prod_map
      (((resetMeasure.prod volume).restrict ageDomain).prod velocityMeasure)
      hazardMeasure (hageMap.prodMap measurable_id) measurable_id,
    ← Measure.map_prod_map
      ((resetMeasure.prod volume).restrict ageDomain) velocityMeasure
      hageMap measurable_id,
    Measure.map_id, Measure.map_id,
...
instance gaussianZigZagNormalizedSuspensionOccupationMeasure.instIsProbabilityMeasure :
  ...
  have hdecoderMass := congrArg (fun measure : Measure
      ((((ℝ × ℝ) × ℝ) × Bool) × (ℕ → NNReal)) => measure Set.univ)
    gaussianZigZagSuspensionDecode_map_occupation_eq_smul
  have hmass : gaussianZigZagSuspensionOccupationMeasure Set.univ =
      gaussianZigZagCycleMeanDuration := by
...
theorem gaussianZigZagSignedHorizonEndpoint_suspensionState
  ...
  | zero =>
      ...
      have hremaining : horizon < gaussianZigZagCycleRemainingTime
        ...
        rw [coe_gaussianZigZagCycleRemainingTime _ _ hcover]
        unfold gaussianZigZagSuspensionState
        linarith [hbefore, hroof]
  ...
  | succ eventCount ih =>
      ...
      · rw [if_pos hbefore]
        have hbeforeReal : input.2 + (horizon : ℝ) <
            gaussianZigZagCyclePhaseEnvironmentRoof input.1 := by
          have hbeforeCoe : (horizon : ℝ) < (remaining : ℝ) := by
            exact_mod_cast hbefore
          rw [hremainingCoe] at hbeforeCoe
          linarith
      ...
      · rw [if_neg hbefore]
        have hreach : gaussianZigZagCyclePhaseEnvironmentRoof input.1 ≤
          ...
          have hremainingLe : (remaining : ℝ) ≤ (horizon : ℝ) := by
            exact_mod_cast not_lt.mp hbefore
          rw [hremainingCoe] at hremainingLe
          linarith
        let shiftedInput :=
        ...
        have hshiftedResets : shiftedInput.1.1.1.1 < 0 ∧
          ...
          exact ⟨hresets.2, by
            ...
            have hhazard : (0 : ℝ) < (input.1.1.2 0 : ℝ) := by
              exact_mod_cast hpositive 0
            linarith⟩
        have hshiftedBelow : shiftedInput.2 <
            gaussianZigZagCyclePhaseEnvironmentRoof shiftedInput.1 := by
          have hhazard : (0 : ℝ) < (input.1.1.2 0 : ℝ) := by
            exact_mod_cast hpositive 0
          have hsqrtPos : 0 < Real.sqrt (2 * (input.1.1.2 0 : ℝ)) :=
            Real.sqrt_pos.2 (by linarith)
          change 0 < (Real.toNNReal
            (-gaussianZigZagNegativeRayleighReset (input.1.1.2 0) -
              input.1.1.1.2) : ℝ)
          rw [Real.coe_toNNReal]
          · unfold gaussianZigZagNegativeRayleighReset
            linarith
          · unfold gaussianZigZagNegativeRayleighReset
            linarith [Real.sqrt_nonneg (2 * (input.1.1.2 0 : ℝ))]
        ...
        have hstream : gaussianZigZagSuspensionHazardStream shiftedInput =
          ...
          | zero =>
              simp only [gaussianZigZagSuspensionHazardStream,
                gaussianZigZagHazardCons]
              dsimp [shiftedInput, gaussianZigZagSuspensionState,
                gaussianZigZagCyclePhaseEnvironmentShift,
                gaussianZigZagCycleEnvironmentShift]
              simp only [add_zero]
        ...
        have hshiftedState : gaussianZigZagSuspensionState shiftedInput =
            (input.1.1.1.2, !(gaussianZigZagSuspensionState input).2) := by
          simp [shiftedInput, gaussianZigZagSuspensionState,
            gaussianZigZagCyclePhaseEnvironmentShift,
            gaussianZigZagCycleEnvironmentShift]
        rw [← hshiftedState, show horizon -
          gaussianZigZagCycleRemainingTime
            (gaussianZigZagSuspensionState input).1 input.1.1.1.2 =
              residual by rfl, hrecursive]
...
theorem gaussianZigZagSuspensionState_map_normalized :
  ...
  have hfactor : gaussianZigZagSuspensionState =
    ...
    exact (congrArg Prod.fst
      (gaussianZigZagStationaryCycleStreamMap_decode input)).symm
  rw [hfactor]
  rw [← Measure.map_map
...
theorem gaussianZigZagSignedHorizonKernel_invariant
  ...
  have hcomposition : execute ∘ gaussianZigZagSuspensionDecode =
      fun input => gaussianZigZagSignedHorizonEndpoint
        ...
        (gaussianZigZagSuspensionHazardStream input) := by
    funext input
    change gaussianZigZagSignedHorizonEndpoint
      (gaussianZigZagStationaryCycleStreamMap
        (gaussianZigZagSuspensionDecode input)).1 horizon
      (gaussianZigZagStationaryCycleStreamMap
        (gaussianZigZagSuspensionDecode input)).2 = _
  ...
  change Measure.map (gaussianZigZagSuspensionState ∘
      suspensionEndpoint gaussianZigZagCyclePhaseEnvironmentShift
        gaussianZigZagCyclePhaseEnvironmentRoof (horizon : ℝ))
      gaussianZigZagNormalizedSuspensionOccupationMeasure = _
