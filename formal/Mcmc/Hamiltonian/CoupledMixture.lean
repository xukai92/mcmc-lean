...
theorem gaussianDensityAcceptance_eq_boltzmannRwmhAcceptance
    ...
    Mcmc.Kernel.densityAcceptance (positionBoltzmannWeight potential)
        (Mcmc.Kernel.randomWalkProposalDensity
          (Mcmc.Kernel.isotropicGaussianPDF variance)) x z =
  ...
  · exact Mcmc.Kernel.isotropicGaussianPDF_even variance
  · intro y
    exact (positionBoltzmannWeight_pos potential y).ne'
  · intro y
    exact (Mcmc.Kernel.isotropicGaussianPDF_pos variance hvariance y).ne'
  · exact Mcmc.Kernel.isotropicGaussianPDF_ne_top variance
...
theorem exists_pos_boltzmannBothAcceptanceFloor_on_compact
  ...
  obtain ⟨acceptanceFloor, hfloorPos, hfloor⟩ :=
    Mcmc.Kernel.exists_pos_le_on_compact (hC.prod hA)
      (hCne.prod hAne)
...
theorem coupledBothAcceptWeight_diagonal_eq_boltzmannBothAcceptance
    ...
    Mcmc.Kernel.coupledBothAcceptWeight
        (Mcmc.Kernel.densityAcceptance (positionBoltzmannWeight potential)
          (Mcmc.Kernel.randomWalkProposalDensity
            (Mcmc.Kernel.isotropicGaussianPDF variance))) current proposal =
  ...
  subst z₂
  unfold Mcmc.Kernel.coupledBothAcceptWeight
    Mcmc.Kernel.bothAcceptWeight Mcmc.Kernel.coupledLeftAcceptance
    Mcmc.Kernel.coupledRightAcceptance boltzmannBothAcceptance
...
instance coupledHmcRwmhMixture_isMarkovKernel
  ...
  letI : IsMarkovKernel
      (Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings
        (positionBoltzmannWeight potential) variance hvariance) :=
    Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings_isMarkov
      _ variance hvariance (measurable_positionBoltzmannWeight hpotential)
  infer_instance
...
theorem coupledHmcRwmhMixture_isCoupling_and_invariant
  ...
  apply Mcmc.Kernel.coupledMixture_isCoupling_and_invariant
  · exact maximalSharedMomentumCoupledPositionMultinomialHMC_isCoupling
      potential gradient ε L hpotential hgradient
  · exact Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings_isCoupling
      (positionBoltzmannWeight potential) variance hvariance
      (measurable_positionBoltzmannWeight hpotential)
  · exact standardPositionMultinomialHMC_invariant hpotential hgradient ε L
  · simpa [positionBoltzmannTarget, Mcmc.Kernel.densityTarget] using
      Mcmc.Kernel.euclideanGaussianRandomWalkMetropolisHastings_invariant
        (positionBoltzmannWeight potential) variance hvariance
        (measurable_positionBoltzmannWeight hpotential)
...
theorem coupledHmcRwmhMixture_meeting_pos
  ...
  unfold coupledHmcRwmhMixture
  apply Mcmc.Kernel.mixture_diagonal_pos_of_second p hp
  exact Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings_meeting_pos
    (positionBoltzmannWeight potential) variance hvariance
    (measurable_positionBoltzmannWeight hpotential)
...
theorem coupledHmcRwmhMixture_isExactMeetingSmallSet
    ...
    (hRwmh : Mcmc.Kernel.IsExactMeetingSmallSet
      (Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings
        (positionBoltzmannWeight potential) variance hvariance)
      C meetingBound) :
    Mcmc.Kernel.IsExactMeetingSmallSet
      (coupledHmcRwmhMixture p potential gradient ε L hpotential hgradient
        variance hvariance)
      C (((1 - p.1 : NNReal) : ENNReal) * meetingBound) := by
  unfold coupledHmcRwmhMixture
  exact Mcmc.Kernel.mixture_isExactMeetingSmallSet_of_second
    p _ _ hRwmh
...
theorem coupledHmcRwmhMixture_isExactMeetingSmallSet_of_floors
    ...
    (hleft : ∀ current ∈ C, ∀ z ∈ A,
      proposalFloor ≤ Mcmc.Kernel.isotropicGaussianPDF variance
        (z - current.1))
    (hright : ∀ current ∈ C, ∀ z ∈ A,
      proposalFloor ≤ Mcmc.Kernel.isotropicGaussianPDF variance
        (z - current.2))
    (haccept : ∀ current ∈ C, ∀ z ∈ Set.diagonal (Position ι),
      acceptanceFloor ≤ Mcmc.Kernel.coupledBothAcceptWeight
        (Mcmc.Kernel.densityAcceptance (positionBoltzmannWeight potential)
          (Mcmc.Kernel.randomWalkProposalDensity
            (Mcmc.Kernel.isotropicGaussianPDF variance))) current z) :
    Mcmc.Kernel.IsExactMeetingSmallSet
      (coupledHmcRwmhMixture p potential gradient ε L hpotential hgradient
        variance hvariance)
  ...
  exact Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings_isExactMeetingSmallSet_of_floors
    (positionBoltzmannWeight potential) variance hvariance
...
theorem coupledHmcRwmhMixture_isExactMeetingSmallSet_of_localFloors
  ...
  apply coupledHmcRwmhMixture_isExactMeetingSmallSet p potential gradient ε L
    hpotential hgradient variance hvariance C
    (acceptanceFloor * (proposalFloor * volume A))
  exact Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings_isExactMeetingSmallSet_of_localFloors
    (positionBoltzmannWeight potential) variance hvariance
    (measurable_positionBoltzmannWeight hpotential) C A hA proposalFloor
    acceptanceFloor hleft hright haccept
...
theorem exists_pos_coupledHmcRwmhMixture_exactMeetingSmallSet_on_compact
  ...
  obtain ⟨proposalFloor, hproposalFloorPos, hleft, hright⟩ :=
    Mcmc.Kernel.exists_pos_euclideanGaussianProposalDensityFloor_on_compact
...
theorem stickyCoupledHmcRwmhMixture_isCoupling
    ...
    Mcmc.Kernel.IsCoupling
      ...
      (hmcRwmhMixture p potential gradient ε L hpotential hgradient variance
        hvariance) := by
  apply Mcmc.Kernel.stickyCoupling_isCoupling
...
theorem stickyCoupledHmcRwmhMixture_isFaithful
    ...
    (hgradient : Measurable gradient) (variance : ℝ≥0)
    (hvariance : variance ≠ 0) :
    Mcmc.Kernel.IsFaithful
      (stickyCoupledHmcRwmhMixture p potential gradient ε L hpotential
        hgradient variance hvariance) := by
  unfold stickyCoupledHmcRwmhMixture
  exact Mcmc.Kernel.stickyCoupling_isFaithful _ _
...
theorem XuTheorem41DriftAssumptions.hmc_hasAffineDrift
    ...
    Mcmc.Kernel.HasAffineDrift hmc h.V h.driftCoefficient
      h.driftAllowance :=
...
theorem XuTheorem41DriftAssumptions.rwmh_hasAffineDrift
    ...
    {S : Set (Position ι)}
    (h : XuTheorem41DriftAssumptions γ hmc rwmh initial potential S) :
    Mcmc.Kernel.HasAffineDrift rwmh h.V h.growthCoefficient
...
theorem XuTheorem41DriftAssumptions.initialCoupling_pairedMoment_ne_top
    ...
    (∫⁻ q, Mcmc.Kernel.IsCoupling.pairedAdd h.V q ∂initialCoupling) ≠ ∞ := by
  have heq :
      (∫⁻ q, Mcmc.Kernel.IsCoupling.pairedAdd h.V q ∂initialCoupling) =
    ...
    rw [lintegral_add_left hfst,
      Mcmc.Kernel.lintegral_fst_eq_of_isMeasureCoupling hinitial
        h.measurable_V,
      Mcmc.Kernel.lintegral_snd_eq_of_isMeasureCoupling hinitial
        h.measurable_V]
  rw [heq]
...
theorem XuTheorem41DriftAssumptions.coupling_hasGeometricDrift
    ...
    (hcoupled : Mcmc.Kernel.IsCoupling coupled
      (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ) hmc rwmh)
      (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ) hmc rwmh)) :
    Mcmc.Kernel.HasGeometricDrift coupled
      (Mcmc.Kernel.IsCoupling.pairedAdd h.V)
      (Mcmc.Kernel.lyapunovSublevel
        (Mcmc.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1))
...
theorem XuTheorem41DriftAssumptions.mixture_relaxedAccessibleFrom_pairedSublevel
    ...
    (coupledHmc coupledRwmh :
      Kernel (Position ι × Position ι) (Position ι × Position ι))
    {δ : ℝ} {entry : ENNReal}
    (haccess : Mcmc.Kernel.IsRelaxedMeetingAccessibleFrom
      coupledHmc S δ 1 entry) :
    Mcmc.Kernel.IsUniformlyAccessibleFrom
      (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ)
        coupledHmc coupledRwmh)
      (Mcmc.Kernel.lyapunovSublevel
        (Mcmc.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1))
      (Mcmc.Kernel.relaxedDiagonal δ) 1
      ((((1 - γ.1 : NNReal) : NNReal) : ENNReal) * entry) := by
  have hlocal : Mcmc.Kernel.IsUniformlyAccessibleFrom coupledHmc
      (Mcmc.Kernel.lyapunovSublevel
        (Mcmc.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1))
      (Mcmc.Kernel.relaxedDiagonal δ) 1 entry := by
    intro q hq
  ...
  simpa only [xuTheorem41HmcWeight] using
    hlocal.mixture_first_one (xuTheorem41HmcWeight γ)
      coupledHmc coupledRwmh
      (Mcmc.Kernel.measurableSet_relaxedDiagonal δ)
...
theorem XuTheorem41DriftAssumptions.exists_geometric_relaxedPairMeetingTail
    ...
    (hhmc : Mcmc.Kernel.IsCoupling coupledHmc hmc hmc)
    (hrwmh : Mcmc.Kernel.IsCoupling coupledRwmh rwmh rwmh)
    ...
    (hx : x ∉ Mcmc.Kernel.relaxedDiagonal δ)
    (hVxTop : Mcmc.Kernel.IsCoupling.pairedAdd h.V x ≠ ∞) :
    ∃ scale contractionRate : ENNReal,
      scale ≠ 0 ∧ scale ≠ ∞ ∧ contractionRate < 1 ∧
        Mcmc.Kernel.meetingWeight
          (Mcmc.Kernel.IsCoupling.pairedAdd h.V) scale x ≠ ∞ ∧
          ∀ n : ℕ,
            Mcmc.Kernel.relaxedPairMeetingTail
                (Mcmc.Kernel.pathKernel
                  (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ)
                    coupledHmc coupledRwmh) x) δ n ≤
              contractionRate ^ n *
                Mcmc.Kernel.meetingWeight
                  (Mcmc.Kernel.IsCoupling.pairedAdd h.V) scale x := by
  let coupled := Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ)
    coupledHmc coupledRwmh
  ...
  have hcoupled : Mcmc.Kernel.IsCoupling coupled
      (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ) hmc rwmh)
      (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ) hmc rwmh) := by
    dsimp only [coupled]
    exact Mcmc.Kernel.mixture_isCoupling
  ...
  have hentry : ∀ q ∈ Mcmc.Kernel.lyapunovSublevel Vpair threshold,
      entry ≤ coupled q (Mcmc.Kernel.relaxedDiagonal δ) := by
    intro q hq
  ...
  have hentryLe : entry ≤ 1 := by
    ...
    calc
      entry ≤ coupled q (Mcmc.Kernel.relaxedDiagonal δ) := hentry q hq
  ...
  obtain ⟨scale, contractionRate, hscale0, hscaleTop, hrate,
    ...
    hdrift.exists_scale_rate_targetHittingTail_pathKernel_le coupled
      (Mcmc.Kernel.measurableSet_relaxedDiagonal δ) hentry hdriftRate
  ...
  calc
    Mcmc.Kernel.relaxedPairMeetingTail
        (Mcmc.Kernel.pathKernel coupled x) δ n ≤
        Mcmc.Kernel.pathKernel coupled x
          (Mcmc.Kernel.returnFailureEvent
            (Mcmc.Kernel.relaxedDiagonal δ) n) := by
      apply measure_mono
      intro path hfail
      rw [Mcmc.Kernel.mem_returnFailureEvent_iff]
      intro j hj1 hjn hclose
      apply hfail
      exact Set.mem_iUnion.mpr ⟨j, Set.mem_iUnion.mpr
        ⟨Finset.mem_range.mpr (Nat.lt_succ_iff.mpr hjn), hclose⟩⟩
    _ ≤ contractionRate ^ n *
        Mcmc.Kernel.meetingWeight Vpair scale x := htail n
      ...
      ∃ hmcEntry : ENNReal, 0 < hmcEntry ∧
        ∃ εbar > 0, ∀ {ε : ℝ} {L : ℕ},
          ...
          (Mcmc.Kernel.lyapunovSublevel
            ...
            (1 + h.ell1)).Nonempty →
          ∀ x : Position ι × Position ι,
            x ∉ Mcmc.Kernel.relaxedDiagonal δ →
            Mcmc.Kernel.IsCoupling.pairedAdd h.V x ≠ ∞ →
            ∃ scale contractionRate : ENNReal,
              scale ≠ 0 ∧ scale ≠ ∞ ∧ contractionRate < 1 ∧
                Mcmc.Kernel.meetingWeight
                  (Mcmc.Kernel.IsCoupling.pairedAdd h.V) scale x ≠ ∞ ∧
...
theorem XuTheorem41DriftAssumptions.exists_geometric_relaxedPairMeetingTail_initial
    ...
    ∃ C₀ contractionRate : ENNReal,
      C₀ ≠ ∞ ∧ contractionRate < 1 ∧
        ∀ n : ℕ,
          Mcmc.Kernel.relaxedPairMeetingTail
              (Mcmc.Kernel.pathLaw
                (Mcmc.Kernel.laggedInitialMeasure initialCoupling
                  (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ)
                    hmc rwmh))
                (Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ)
  ...
  let transition := Mcmc.Kernel.mixture (xuTheorem41HmcWeight γ) hmc rwmh
  let laggedInitial := Mcmc.Kernel.laggedInitialMeasure
  ...
  let C₀ := ∫⁻ q in (Mcmc.Kernel.relaxedDiagonal δ)ᶜ,
    Mcmc.Kernel.meetingWeight Vpair scale q ∂laggedInitial
  have hsourceRateTop :
      xuTheorem41Lambda0 γ h.driftCoefficient h.growthCoefficient ≠ ∞ :=
  ...
  have hfullWeightTop :
      (∫⁻ q, Mcmc.Kernel.meetingWeight Vpair scale q
        ∂laggedInitial) ≠ ∞ := by
...
theorem XuTheorem41DriftAssumptions.exists_geometric_exactLagOneMeetingTail_initial
    ...
    (hmeeting : Mcmc.Kernel.IsExactMeetingSmallSet
      coupled
      (Mcmc.Kernel.lyapunovSublevel
        (Mcmc.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1))
      meetingBound)
  ...
  have hfaithful' : Mcmc.Kernel.IsFaithful coupled := by
    exact hfaithful
...
theorem stickyCoupledHmcRwmhMixture_hasGeometricDrift_pairedAdd_of_components
    ...
    (hhmc : Mcmc.Kernel.HasGeometricDrift
      (maximalSharedMomentumCoupledPositionMultinomialHMC
        potential gradient ε L hpotential hgradient)
      (Mcmc.Kernel.IsCoupling.pairedAdd v) C hmcRate hmcAllowance)
    (hrwmh : Mcmc.Kernel.HasGeometricDrift
      (Mcmc.Kernel.coupledEuclideanGaussianRandomWalkMetropolisHastings
        (positionBoltzmannWeight potential) variance hvariance)
      (Mcmc.Kernel.IsCoupling.pairedAdd v) C rwmhRate rwmhAllowance) :
    Mcmc.Kernel.HasGeometricDrift
      (stickyCoupledHmcRwmhMixture p potential gradient ε L hpotential
        hgradient variance hvariance)
      (Mcmc.Kernel.IsCoupling.pairedAdd v) C
      (hmcRwmhDriftRate p hmcRate rwmhRate)
      (hmcRwmhDriftAllowance p hmcAllowance rwmhAllowance) := by
  have hmixture := Mcmc.Kernel.HasGeometricDrift.mixture p
    (maximalSharedMomentumCoupledPositionMultinomialHMC
...
theorem stickyCoupledHmcRwmhMixture_hasGeometricDrift_of_singleChain
    ...
    (hhmc : Mcmc.Kernel.HasAffineDrift
      (standardPositionMultinomialHMC potential gradient ε L
        hpotential hgradient)
      v hmcSourceRate hmcSourceAllowance)
    (hrwmh : Mcmc.Kernel.HasAffineDrift
      (Mcmc.Kernel.euclideanGaussianRandomWalkMetropolisHastings
        (positionBoltzmannWeight potential) variance hvariance)
    ...
    Mcmc.Kernel.HasGeometricDrift
      (stickyCoupledHmcRwmhMixture p potential gradient ε L hpotential
        hgradient variance hvariance)
      (Mcmc.Kernel.IsCoupling.pairedAdd v)
      (Mcmc.Kernel.lyapunovSublevel
        (Mcmc.Kernel.IsCoupling.pairedAdd v) threshold)
...
theorem exists_pos_stickyCoupledHmcRwmhMixture_exactMeetingSmallSet_on_compact
  ...
  exact ⟨meetingBound, hboundPos,
    Mcmc.Kernel.stickyCoupling_isExactMeetingSmallSet _ _ hsmall⟩
...
theorem XuTheorem41DriftAssumptions.exists_geometric_exactLagOneMeetingTail_stickyHmcRwmh
  ...
  let C := Mcmc.Kernel.lyapunovSublevel
    (Mcmc.Kernel.IsCoupling.pairedAdd h.V) (1 + h.ell1)
  have hp : p.1 < 1 := by
...
theorem exists_scale_rate_stickyCoupledHmcRwmhMixture_meetingTail_of_component_drift
    ...
    (hhmc : Mcmc.Kernel.HasGeometricDrift
      (maximalSharedMomentumCoupledPositionMultinomialHMC
        potential gradient ε L hpotential.measurable hgradient)
      V (Mcmc.Kernel.lyapunovSublevel V threshold)
    ...
    (hdiagonal : ∀ x ∈ Set.diagonal (Position ι),
      (∫⁻ y, V y ∂Mcmc.Kernel.synchronousCoupling
        ...
        hmcRwmhDriftRate p hmcRate rwmhRate * V x +
          (Mcmc.Kernel.lyapunovSublevel V threshold).indicator
    ...
    (hsubCompact : IsCompact (Mcmc.Kernel.lyapunovSublevel V threshold))
    (hsubNonempty :
      (Mcmc.Kernel.lyapunovSublevel V threshold).Nonempty)
    {A : Set (Position ι)}
    (hA : IsCompact A) (hAne : A.Nonempty)
    (hAmeas : MeasurableSet A) (hAvolume : 0 < volume A)
    ...
    ∃ scale contractionRate : ENNReal,
      scale ≠ 0 ∧ scale ≠ ∞ ∧ contractionRate < 1 ∧
        Mcmc.Kernel.meetingWeight V scale x ≠ ∞ ∧
          ∀ n : ℕ,
            Mcmc.Kernel.exactMeetingTail
                (Mcmc.Kernel.pathKernel
                  (stickyCoupledHmcRwmhMixture p potential gradient ε L
              ...
              contractionRate ^ n *
                Mcmc.Kernel.meetingWeight V scale x := by
  let coupled := stickyCoupledHmcRwmhMixture p potential gradient ε L
    hpotential.measurable hgradient variance hvariance
  have hdrift : Mcmc.Kernel.HasGeometricDrift coupled V
      (Mcmc.Kernel.lyapunovSublevel V threshold)
      (hmcRwmhDriftRate p hmcRate rwmhRate)
      (hmcRwmhDriftAllowance p hmcAllowance rwmhAllowance) :=
...
theorem exists_scale_rate_stickyCoupledHmcRwmhMixture_meetingTail_pairedAdd
    ...
    (hbudgetTop :
      hmcRwmhDriftRate p hmcRate rwmhRate * threshold +
        hmcRwmhDriftAllowance p hmcAllowance rwmhAllowance ≠ ∞)
    (hsubCompact : IsCompact (Mcmc.Kernel.lyapunovSublevel
      (Mcmc.Kernel.IsCoupling.pairedAdd v) threshold))
    (hsubNonempty : (Mcmc.Kernel.lyapunovSublevel
      (Mcmc.Kernel.IsCoupling.pairedAdd v) threshold).Nonempty)
    {A : Set (Position ι)}
    (hA : IsCompact A) (hAne : A.Nonempty)
    (hAmeas : MeasurableSet A) (hAvolume : 0 < volume A)
    (x : Position ι × Position ι)
    (hVxTop : Mcmc.Kernel.IsCoupling.pairedAdd v x ≠ ∞) :
    ∃ scale contractionRate : ENNReal,
      scale ≠ 0 ∧ scale ≠ ∞ ∧ contractionRate < 1 ∧
        Mcmc.Kernel.meetingWeight
          (Mcmc.Kernel.IsCoupling.pairedAdd v) scale x ≠ ∞ ∧
          ∀ n : ℕ,
            Mcmc.Kernel.exactMeetingTail
                (Mcmc.Kernel.pathKernel
                  (stickyCoupledHmcRwmhMixture p potential gradient ε L
              ...
              contractionRate ^ n *
                Mcmc.Kernel.meetingWeight
                  (Mcmc.Kernel.IsCoupling.pairedAdd v) scale x := by
  let V := Mcmc.Kernel.IsCoupling.pairedAdd v
  let coupled := stickyCoupledHmcRwmhMixture p potential gradient ε L
    hpotential.measurable hgradient variance hvariance
  have hdrift : Mcmc.Kernel.HasGeometricDrift coupled V
      (Mcmc.Kernel.lyapunovSublevel V threshold)
          ...
          ∀ n : ℕ,
            Mcmc.Kernel.exactMeetingTail
              ...
              contractionRate ^ n *
                Mcmc.Kernel.meetingWeight
                  (Mcmc.Kernel.IsCoupling.pairedAdd v) scale x := by
  let hmcPairedRate := Mcmc.Kernel.affinePairedSublevelRate
    hmcRate hmcAllowance threshold
  let rwmhPairedRate := Mcmc.Kernel.affinePairedSublevelRate
    rwmhRate rwmhAllowance threshold
  obtain ⟨hhmcRates, hhmcThreshold, hhmcPairedRate⟩ :=
    Mcmc.Kernel.affinePairedSublevelRate_spec hthreshold0 hthresholdTop
      hhmcBudget
...
theorem exists_pos_stickyCoupledHmcRwmhMixture_skeleton_geometric
    ...
    (hreturn : Mcmc.Kernel.IsUniformlyAccessible
      (stickyCoupledHmcRwmhMixture p potential gradient ε L
        hpotential.measurable hgradient variance hvariance)
      C steps returnBound) :
    ∃ meetingBound : ENNReal, 0 < meetingBound ∧
      ...
      ∀ n,
        Mcmc.Kernel.offDiagonalMassAtTime initial
            (stickyCoupledHmcRwmhMixture p potential gradient ε L
              hpotential.measurable hgradient variance hvariance)
            ((steps + 1) * n) ≤
  ...
  exact Mcmc.Kernel.offDiagonalMassAtSkeletonTime_le_geometric
    initial
    (stickyCoupledHmcRwmhMixture p potential gradient ε L
      hpotential.measurable hgradient variance hvariance)
...
theorem exists_pos_stickyCoupledHmcRwmhMixture_meetingTail_geometric
  ...
  refine ⟨meetingBound, hmeetingPos, hproduct,
    ENNReal.sub_lt_self (by simp) (by simp) hproduct.ne', fun n => ?_⟩
  exact Mcmc.Kernel.exactMeetingTail_pathLaw_le_geometric_div
    initial
    (stickyCoupledHmcRwmhMixture p potential gradient ε L
      hpotential.measurable hgradient variance hvariance)
    hCmeas steps returnBound meetingBound hreturn hmeeting
...
theorem exists_pos_stickyCoupledHmcRwmhMixture_meetingTail_of_bounded_drift
    ...
    (hdrift : Mcmc.Kernel.HasGeometricDrift
      (stickyCoupledHmcRwmhMixture p potential gradient ε L
        hpotential.measurable hgradient variance hvariance)
    ...
    (hsubCompact : IsCompact (Mcmc.Kernel.lyapunovSublevel V R))
    (hsubNonempty : (Mcmc.Kernel.lyapunovSublevel V R).Nonempty)
    {A : Set (Position ι)}
    (hA : IsCompact A) (hAne : A.Nonempty)
    (hAmeas : MeasurableSet A) (hAvolume : 0 < volume A) :
    ∃ meetingBound : ENNReal, 0 < meetingBound ∧
