...
theorem IsCompact.exists_uniform_offsetLeapfrog_positionNorm_bound
  ...
  have hpositions :=
    Mcmc.Hamiltonian.IsCompact.offsetLeapfrogPositionSet
      hgradient ε origin hK
  rcases Mcmc.Hamiltonian.IsCompact.exists_euclideanNorm_bound hpositions with
    ⟨R, hR, hbound⟩
  refine ⟨R, hR, ?_⟩
  intro z hz i
  exact hbound _
...
theorem RegularPotential.exists_uniform_offsetLeapfrog_positionNorm_bound
  ...
  Mcmc.Hamiltonian.IsCompact.exists_uniform_offsetLeapfrog_positionNorm_bound
...
theorem RegularPotential.exists_uniform_offsetLeapfrog_phaseSize_bound_of_horizon
  ...
  rcases Mcmc.Hamiltonian.IsCompact.exists_euclideanPhaseSize_bound hK with
    ⟨M, hM, hMbound⟩
...
theorem IsCompact.exists_trajectorySquaredPositionCost_bound
    ...
    ∃ R > 0, ∀ i j,
      trajectorySquaredPositionCost gradient ε z origin i j ≤
        ⟨4 * R ^ 2, by positivity⟩ := by
  rcases Mcmc.Hamiltonian.IsCompact.exists_euclideanNorm_bound hS with
    ⟨R, hR, hbound⟩
  refine ⟨R, hR, ?_⟩
  intro i j
...
theorem totalVariation_tendsto_zero_of_le_expSqBound
    ...
    (hbound : ∀ᶠ k in l,
      Mcmc.Finite.totalVariation (p k) (q k) ≤
        (ENNReal.ofReal (Real.exp (radius k))) ^ 2 - 1) :
    Filter.Tendsto
      (fun k => Mcmc.Finite.totalVariation (p k) (q k))
      l (nhds 0) := by
...
theorem randomizedMaximalTrajectoryIndexCoupling_mismatchMass
  ...
  congr 1
  exact Mcmc.Finite.IsMaximalCoupling.mismatchMass_eq_totalVariation
...
theorem xuProposition42TVConclusion_iff_maximalMismatch
  ...
  · intro h δ hδ
    ...
    refine ⟨ε0, hε0, L0, hL0, ?_⟩
    intro ε hεpos hε L hlength
    have hmax := Mcmc.Finite.maximalCoupling_isMaximal
      (trajectoryIndexPMF potential (trajectory₁ ε L))
      (trajectoryIndexPMF potential (trajectory₂ ε L))
...
theorem trajectoryIndexPMF_totalVariation_tendsto_zero_of_energy
    ...
    Filter.Tendsto
      (fun k => Mcmc.Finite.totalVariation
