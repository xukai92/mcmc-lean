...
The maintained overview is now the
[Lean-generated formalization dependency graph](generated/architecture-graphs.md#Formalization-dependency-graph).
Its nodes and edges are definitions in `Mcmc.Docs.Graph`, so they are
typechecked with the formal project and regenerated by `make generate-docs`.

The more detailed branch-level diagram below remains useful as explanatory
documentation.
