...
| M2. Finite particle Gibbs | Complete | `particleGibbsKernel` composes conditional SMC with uniform terminal-index refresh; `particleGibbsKernel_stationary` and `particleGibbs_stationary_selectedTrajectory_expectation` prove extended-target stationarity and the exact normalized Feynman--Kac trajectory marginal. The zero-horizon specialization is exactly `N⁻¹ I + (1-N⁻¹) Π`, with exact `N⁻ᵏ` total-variation contraction and convergence for `N ≥ 2`; `N = 1` is exactly the identity. `Mcmc.Examples.ParticleGibbs` is a compiled client. | General positive-horizon convergence rates and consistency as particle count grows are not claimed. |
...
- Particle Gibbs is exact for the finite extended target under the hypotheses
  ...
  claimed.
- The corrected Xu--Ge position-dependent execution path is conditional on its
  solver certificate. The deferred bounded floating-point refinement layer is
  still explicit.
