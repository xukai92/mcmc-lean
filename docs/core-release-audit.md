...
| M2. Finite particle Gibbs | Complete | `particleGibbsKernel` composes conditional SMC with uniform terminal-index refresh; `particleGibbsKernel_stationary` and `particleGibbs_stationary_selectedTrajectory_expectation` prove extended-target stationarity and the exact normalized Feynman--Kac trajectory marginal. `Mcmc.Examples.ParticleGibbs` is a compiled client. | This is fixed-particle, finite-horizon exactness, not convergence from arbitrary starts or consistency as particle count grows. |
...
- Particle Gibbs is exact for the finite extended target under the hypotheses
  above; no mixing-rate or particle-efficiency theorem is claimed.
