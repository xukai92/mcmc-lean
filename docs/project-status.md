...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. A public randomized recursive-doubling HMC client now samples Boolean direction traces from the Lean-generated descriptor; Lean proves the state-independent mixture stationary even when individual traces fail certification and become identity moves. Equivalence with ordinary randomized root-dependent standard NUTS remains open; the two-leaf obstruction rules out naive universal certification. |
