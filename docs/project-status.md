...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, checked-or-identity execution, eligible-count streaming, finite auxiliary-law stationarity, recursive flag/callback refinement, and linked leapfrog error certificates. The finite foundation supplies endpoint-dependent complete-trace reversal and the C.4 admissible-root construction. Julia now has both `CompletedTreeC4DynamicHMC` and a separately labelled production-shaped fixed-parameter `NUTS` runtime covering classic/generalized/strict-generalized termination, multinomial/slice selection, and ordinary/jittered/tempered leapfrog over every maintained fixed metric. Lean proves that online early-exit recursion on a precomputed phase tree returns the same continuation bit as completed-tree checking and that success produces exactly the completed ordered candidate occurrences; any deterministic consumer, including selection under a fixed random trace, therefore agrees. The fuel-bounded midpoint recursion used by the generated descriptor is proved equal to this structural aggregation on balanced intervals, including arbitrary leaf eligibility/divergence checks and exact consecutive candidates after success. This does not yet connect Julia's concrete call trace or complete runtime transition to the invariant C.4 kernel. The U-turn refinement has a four-leaf nonzero-error (`1/10`) kernel instantiation and correctly handles self-pairs without impossible separation assumptions. Both an exact-dyadic `ε=1/2` orbit and a genuinely rounded `ε=0.1` Float64 Gaussian orbit are instantiated: Lean proves every distinct endpoint margin and full checked-kernel refinement, while Julia checks each rounded step, exact endpoint linkage, all off-diagonal margins, and tamper rejection. Compiled-source refinement, arbitrary rounded trajectories, and comparison with the complete production-shaped transition remain. |
