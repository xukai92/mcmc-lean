...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible traces. Flat phase arrays build balanced recursive trees whose divergence/slice and vector U-turn checks instantiate stopping admissibility. Slice eligibility has exact weights/counts, and all inner/outer streaming merges equal normalized selection over retained endpoints, including empty segments. The complete-tree conditional transition is now assembled on the C.4-admissible root subtype: reversed trace membership and target-mass normalizer equality imply endpoint flow, detailed balance, and stationarity for every positive target. Remaining original/production NUTS work is composition with the state-dependent completed-tree and slice law—or an exact multinomial-production refinement—and runtime representative-draw correspondence; candidate-row equality remains an invalid shortcut. |
