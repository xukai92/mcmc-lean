...
|---|---|
| Classical Gaussian RMHMC | Formal and certificate-gated executable core complete. A momentum-law-independent generalized-HMC theorem proves endpoint Metropolis reversibility/invariance and lift/evolve/project position invariance. The classical specialization proves inverse-factor transport gives `N(0,G(q))`, including the determinant density, and that its phase weight is the Girolami--Calderhead Hamiltonian weight up to a global Gaussian prefactor. Artifact version 21 supplies generated Reference and independent Optimized execution through public `ClassicalRMHMC`, with differential and Gaussian-moment tests. Arbitrary nonconstant metrics still require a valid exact implicit solver; finite-tolerance refinement remains explicit. |
