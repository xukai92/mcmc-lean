...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. A public randomized recursive-doubling HMC client samples Boolean direction traces from the Lean-generated descriptor; Lean proves the checked state-independent mixture stationary. For ordinary root-dependent construction, Lean now provides the correct richer certificate: a bijective reversal of complete rooted traces with equality of target-weighted construction and selection probabilities implies reversibility and stationarity. Instantiating that certificate for the full standard NUTS recursion remains open; the two-leaf obstruction still rules out candidate-row equality as a universal shortcut. |
| Practical slice sampling | Exact disintegration, interval kernels, randomized horizontal updates, joint trace reversal, guarded success, and literal ideal-real finite stepping-out/shrinkage semantics are present, including distinct malformed-trace failure and budget-exhaustion identity behavior. Lean now proves both signed allocation-shift cases, equality of the actually stopped forward/rerooted bracket, shrink-trace likelihood symmetry, and their end-to-end successful-trace likelihood assembly. Accepted-point restricted measure preservation and Haar offset preservation are also checked. Packaging the allocation/rejected-point strata into one restricted product-measure theorem remains; platform-specific callback, `log`, RNG, and arithmetic bounds remain explicit inputs. |
...
| PDMP foundations | General infrastructure and the stationary-suspension theorem are present. Exact Gaussian Zig-Zag nonexplosion/stationarity and unit-speed one-dimensional Gaussian BPS stationarity are complete. For general unbounded rates, Lean now has a proof-bearing inverse-integrated-hazard clock, exact exponential-hazard event kernel and finite event iterates; finite-dimensional BPS data are tied directly to the integral of the canonical rate along linear flow. A concrete multidimensional inverse (including zero-velocity handling), horizon stopping, nonexplosion, stationarity, semigroup, and convergence remain open. |
...
5. Instantiate the inverse-integrated-hazard interface for a concrete
   multidimensional BPS, including its zero-velocity branch; then construct
