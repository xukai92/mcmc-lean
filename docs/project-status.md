...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. A public randomized recursive-doubling HMC client samples Boolean direction traces from the Lean-generated descriptor; Lean proves the checked state-independent mixture stationary. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible traces. Recursive leaf-divergence, child, and join-U-turn flags now compute a proved outer first-failure continuation depth; failed final doublings are excluded, and the existing recursive barrier tree succeeds exactly when no join is blocked. Normalized endpoint selection and count-proportional streaming merge are proved for all nonnegative counts, including empty subtrees. Remaining standard-NUTS work is to prove that the production trajectory builder's concrete divergence/vector endpoint tests generate these per-root flag trees and retained endpoint weights; the two-leaf obstruction still rules out candidate-row equality as a universal shortcut. |
