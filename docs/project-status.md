...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible traces. Recursive phase-point trees now compute actual leaf slice/divergence predicates and the existing Euclidean vector endpoint U-turn test at every join; Lean proves their continuation result exactly characterizes success of every check and instantiates per-root stopping admissibility. Failed final doublings are excluded. Normalized endpoint selection and count-proportional streaming merge are proved for all nonnegative counts, including empty subtrees. Remaining standard-NUTS work is to prove correspondence with the flat production recursive-doubling trajectory and retained endpoint weights, then assemble the rooted-trace certificate; the two-leaf obstruction still rules out candidate-row equality as a shortcut. |
