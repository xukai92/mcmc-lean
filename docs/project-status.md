...
|---|---|
| Classical Gaussian RMHMC | Formal core complete. A momentum-law-independent generalized-HMC theorem proves endpoint Metropolis reversibility/invariance and lift/evolve/project position invariance. The classical specialization proves inverse-factor transport gives `N(0,G(q))`, including the determinant density, and that its phase weight is the Girolami--Calderhead Hamiltonian weight up to a global Gaussian prefactor. The identity-metric client discharges every obligation. Arbitrary nonconstant metrics still require a valid exact implicit solver, and a maintained general Julia sampler is pending. |
