...
|---|---|
| Classical Gaussian RMHMC | Formal and certificate-gated executable core complete. A momentum-law-independent generalized-HMC theorem proves endpoint Metropolis reversibility/invariance and lift/evolve/project position invariance. The classical specialization proves inverse-factor transport gives `N(0,G(q))`, including the determinant density, and that its phase weight is the Girolami--Calderhead Hamiltonian weight up to a global Gaussian prefactor. Artifact version 22 supplies exact-certificate and bounded-residual Reference commands. Public `ClassicalRMHMC` retains the exact boundary; generic `DenseRiemannianRMHMC` evaluates arbitrary dense metrics and derivative tensors with fixed-point generalized leapfrog, while explicitly making only a numerical bounded-residual claim. Both public samplers and the Optimized path preserve `T<:AbstractFloat`; generated Reference conversion remains a local Float64 boundary. |
