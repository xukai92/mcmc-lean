...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, checked-or-identity execution, eligible-count streaming, finite auxiliary-law stationarity, recursive flag/callback refinement, and linked leapfrog error certificates. The finite foundation supplies endpoint-dependent complete-trace reversal and the C.4 admissible-root construction. Julia now has `CompletedTreeC4DynamicHMC`: it builds a randomized-origin power-of-two orbit, uses root-specific reconstruction directions, forms the C.4-admissible component, and performs target-weighted selection; its seeded Gaussian diagnostic moves while the older per-direction checker remains an explicit identity fallback. A canonical coherent-subrow safety utility is also proved and tested but deliberately not substituted for C.4 because one-sided rows reduce to singletons. Lean proves that its canonical `doublingRootEquiv` is exactly the zero-based least-significant-bit-first encoding; IR version 19 emits that convention with proved decoder semantics, and Julia's tested `completed_tree_direction_trace` rejects a mismatched descriptor. The U-turn refinement has a four-leaf nonzero-error (`1/10`) kernel instantiation and correctly handles self-pairs without impossible separation assumptions. Both an exact-dyadic `ε=1/2` orbit and a genuinely rounded `ε=0.1` Float64 Gaussian orbit are instantiated: Lean proves every distinct endpoint margin and full checked-kernel refinement, while Julia checks each rounded step, exact endpoint linkage, all off-diagonal margins, and tamper rejection. Compiled-source refinement, arbitrary rounded trajectories, and comparison with ordinary recursive NUTS stopping semantics remain. |
