...
| Practical slice sampling | Exact disintegration, interval kernels, randomized horizontal updates, joint trace reversal, guarded success, and literal ideal-real finite stepping-out/shrinkage semantics are present, including distinct malformed-trace failure and budget-exhaustion identity behavior. Lean proves shrink-trace likelihood symmetry, accepted-point restricted measure preservation, and aligned-prefix absorption for actually stopped left/right expansions. The two signed allocation-shift cases and final joint trace assembly remain; platform-specific callback, `log`, RNG, and arithmetic bounds remain inputs rather than asserted facts. |
