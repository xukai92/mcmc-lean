...
|---|---|
| Classical Gaussian RMHMC | Formal and certificate-gated executable core complete. A momentum-law-independent generalized-HMC theorem proves endpoint Metropolis reversibility/invariance and lift/evolve/project position invariance. The classical specialization proves inverse-factor transport gives `N(0,G(q))`, including the determinant density, and that its phase weight is the Girolami--Calderhead Hamiltonian weight up to a global Gaussian prefactor. Artifact version 23 supplies exact-certificate, bounded-residual dense, and structured random-sketch Reference commands. Public `ClassicalRMHMC` retains the exact boundary; generic `DenseRiemannianRMHMC` and `RandomSketchRMHMC` explicitly make only numerical bounded-residual claims. Reference programs own transition control while metric/curvature and finite-tolerance solver callbacks remain host boundaries. Optimized paths preserve `T<:AbstractFloat`; generated Reference conversion remains a local Float64 boundary. |
...
| Execution backends | Julia exposes independent execution class, evidence kind, and capability fields. Maintained Reference, Optimized, parallel-CPU, host-batch, and scoped broadcast-accelerator descriptors fail closed on unsupported capabilities. Deterministic replay compares values or matching failures plus event consumption; explicit-seed threaded chains reproduce sequential chains in stable order. Batched Gaussian RWMH takes explicit events and matches scalar Reference on host arrays. This is not a claim about a particular GPU runtime, device arithmetic, or numerical refinement. |
| Diagnostics | Shared targets, replay, optimization records, and statistical formulas live in `VerifiedSamplers.Evaluation`. Integrated tests use them as regression gates; benchmark workloads add timing, multi-chain aggregation, and reports. Statistical, finite-difference, and performance tests are diagnostics, not replacements for formal or refinement theorems. |
