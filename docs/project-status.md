...
| Practical slice sampling | Exact disintegration, interval kernels, randomized horizontal updates, joint trace reversal, guarded success, runtime stepping-out/shrinkage, and exhaustion fallback are present. Lean proves bounded agreement of every finite-precision stepping-out/shrinkage comparison from callback/log-height error bounds and strict margins, and Julia checks the matching execution certificate. The concrete success transform still needs restricted joint-law preservation; platform-specific callback, `log`, RNG, and arithmetic bounds remain inputs rather than asserted facts. |
