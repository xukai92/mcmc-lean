...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate C.4; flat phase arrays build balanced recursive trees whose divergence/slice and vector U-turn checks instantiate stopping admissibility. Slice eligibility has exact weights/counts, and all streaming merges equal normalized selection over retained endpoints, including empty segments. The complete-tree conditional transition on C.4-admissible roots has proved endpoint flow, detailed balance, and stationarity. A new outer augmentation theorem now collapses any state-dependent completed-tree/slice law: normalized conditional reversibility on every positive auxiliary fiber implies marginal detailed balance/stationarity, while null fibers vanish; exact auxiliary Gibbs refresh is fully instantiated. Remaining production NUTS work is embedding each varying admissible-root subtype into the common phase state with its specified fallback and verifying the production tree/draw implementation against the certificate; candidate-row equality remains invalid. |
