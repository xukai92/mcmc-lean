...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. A public randomized recursive-doubling HMC client samples Boolean direction traces from the Lean-generated descriptor; Lean proves the checked state-independent mixture stationary. For ordinary root-dependent construction, Lean now provides the correct richer certificate: a bijective reversal of complete rooted traces with equality of target-weighted construction and selection probabilities implies reversibility and stationarity. The construction obligation composes from local random-choice reversals. Exact completed-tree combinatorics now instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d possible roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible rerooted traces. Deterministic continuation depth filters roots that stop too early. Normalized endpoint selection and count-proportional streaming merge are proved for all nonnegative counts, including zero-eligible subtrees. Remaining standard-NUTS work is to identify the concrete recursive divergence/U-turn flags and final-doubling exclusion with that stopping-data interface; the two-leaf obstruction still rules out candidate-row equality as a universal shortcut. |
