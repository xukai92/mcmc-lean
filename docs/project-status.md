...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. A public randomized recursive-doubling HMC client samples Boolean direction traces from the Lean-generated descriptor; Lean proves the checked state-independent mixture stationary. For ordinary root-dependent construction, Lean now provides the correct richer certificate: a bijective reversal of complete rooted traces with equality of target-weighted construction and selection probabilities implies reversibility and stationarity. The construction obligation composes from a finite product of local random-choice reversal identities, and the actual bounded fair Boolean direction-trace law is instantiated: every endpoint-dependent bijective reversal preserves its exact mass. Normalized endpoint selection is discharged generically from reversed-trace normalizer equality and pointwise target-weighted endpoint flow. The paper's count-proportional streaming subtree merge is exact for all nonnegative counts: zero-eligible subtree representatives are ignored, positive combined subtrees have the normalized union law, and merging is associative across recursive tree shapes. Remaining standard-NUTS work is the complete reversal of continuation/U-turn flags and the outer state-dependent stopping sequence; the two-leaf obstruction still rules out candidate-row equality as a universal shortcut. |
