...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible traces. Flat power-of-two phase arrays now definitionally build the balanced recursive phase trees used by the proof; their concrete leaf divergence/slice predicates and Euclidean vector endpoint tests instantiate per-root stopping admissibility, with failed final doublings excluded. Slice eligibility is separately represented by zero/one endpoint weights, an exact count, positivity from root retention, and the normalized retained-endpoint law. Count-proportional streaming merge is proved for all nonnegative counts, including empty subtrees. Remaining standard-NUTS work is to prove the outer production streaming draws refine this flat normalized law and discharge the complete trace endpoint-flow identity; candidate-row equality remains an invalid shortcut. |
