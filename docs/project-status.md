...
| Dynamic NUTS | Complete for conservative certified dynamic trees, stopped doubling, all-scales/adjacent builders, checked-or-identity execution, and replay tests. For ordinary root-dependent construction, Lean provides the richer complete-trace reversal certificate. Exact completed-tree combinatorics instantiate the paper's C.4 argument: height-d direction traces biject with the 2^d roots, each root has one reconstruction trace of mass 2^-d, and an endpoint-dependent permutation exchanges compatible traces. Flat power-of-two phase arrays definitionally build balanced recursive phase trees; concrete leaf divergence/slice predicates and Euclidean vector endpoint tests instantiate per-root stopping admissibility, with failed final doublings excluded. Slice eligibility has exact endpoint weights, counts, and normalized laws. A proved `WeightedRepresentative` invariant now shows all inner and outer count-proportional streaming merges, including empty segments, equal normalized selection over the union of retained endpoint weights. Remaining standard-NUTS work is the final complete-trace endpoint-flow identity against the slice-augmented phase target and executable correspondence for runtime representative draws; candidate-row equality remains an invalid shortcut. |
