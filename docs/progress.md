...
| Isotropic MALA | Proved as state-dependent Gaussian MH | Proved reversible and invariant under explicit measurability/finiteness assumptions | No geometric-convergence claim | Scalar/vector IR Reference + generic Optimized | Lean-emitted proposal/acceptance program; Julia Float64 refinement remains explicit |
| Dense position-dependent MALA | Corrected inverse-metric drift, explicit Gaussian density, and normalized-proposal interface | Markov, reversible, and invariant when the concrete density's measurability/normalization obligations hold | — | IR Reference + generic Optimized | Dense Gaussian analytic normalization remains a client hypothesis; Julia factorization/callback/Float boundary is tested, not proved |
| Transport HMC | Exact measurable-equivalence conjugation | Invariant conditional on the exact target-pushforward identity | — | IR-backed Reference + generic Optimized endpoint HMC | Concrete Jacobian identity, callback consistency, and endpoint command refinement remain explicit boundaries |
| Fixed likelihood-informed HMC | Exact common-target composition theorem for active and complement kernels | Conditional on separate active-HMC and complement-pCN invariance clients | — | Reference-composed + generic Optimized | Concrete component theorems and a typed composition descriptor remain open; fixed orthonormal basis only |
...
| Checked dynamic-tree HMC / `NUTS` | Proved for anchored recursive rows, fair bounded traces, checked-or-identity selection, momentum refresh, and projection | Proved for the exact-real Euclidean U-turn Reference kernel; invalid certificates fall back to identity | No generic NUTS convergence claim | Lean-IR-interpreted `NUTS` plus conservative, C.4, and first-stop clients | Float64 trajectory/callback refinement remains explicit; production-NUTS equivalence parked |
| `Optimized.NUTS` production-shaped comparator | Runtime semantics only | Not yet proved for the runtime transition | — | Classic/generalized/strict-generalized × multinomial/slice × ordinary/jittered/tempered, all fixed metrics, depth/divergence diagnostics | Certified decisions refine online candidates; balanced index recursion including leaf failures equals structural continuation; Julia call-trace and full-transition correspondence remain |
