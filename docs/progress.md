...
| Checked dynamic-tree HMC / `VerifiedNUTS` | Proved for anchored recursive rows, fair bounded traces, checked-or-identity selection, momentum refresh, and projection | Proved for the exact-real Euclidean U-turn Reference kernel; invalid certificates fall back to identity | No generic NUTS convergence claim | Lean-IR-interpreted `VerifiedNUTS` plus conservative, C.4, and first-stop clients | Float64 trajectory/callback refinement remains explicit; production-NUTS equivalence parked |
