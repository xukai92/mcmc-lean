...
Artifact version 23 gives generic dense RMHMC an explicit
`dense_rmhmc_step!` Reference entry, and gives random-sketch RMHMC a separate
`random_sketch_rmhmc_step!` entry backed by the `structured-rmhmc` primitive.
Their IR owns stochastic transition control, residual checking, and endpoint
acceptance. This does not encode arbitrary user metric algebra inside the IR:
dense metric, sketch curvature, derivative, and finite-tolerance integrator
callbacks remain declared Julia host boundaries. Thus the programs are
IR-backed executable References, not proofs of callback or Float64 semantics.
