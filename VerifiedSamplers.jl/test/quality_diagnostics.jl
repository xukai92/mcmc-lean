@testset "shared sampling-quality diagnostics" begin
    ...
    @test Evaluation.autocorrelation_ess(constant_values) == 0.0
    alternating = repeat([-1.0, 1.0], 20)
    @test Evaluation.autocorrelation_ess(alternating) == length(alternating)

    chain = [repeat([-1.0, 1.0], 50)'; repeat([-2.0, 2.0], 50)']
    moments = Evaluation.moment_diagnostics(
    ...
    @test Evaluation.marginal_quantile_max_error(
        chain, probabilities, expected) == 0.0

    standard_errors = Evaluation.batch_mean_standard_error(
        chain; batches=10)
    @test length(standard_errors) == 2
    @test all(isfinite, standard_errors)
    @test_throws ArgumentError Evaluation.moment_diagnostics(
        chain, zeros(2), [1.0, 0.0])
    @test_throws DimensionMismatch Evaluation.covariance_max_error(
    ...
    rank_diagnostics = Evaluation.split_rank_diagnostics(independent)
    ...
    shifted[:, 4] .+= 3
    @test Evaluation.split_rank_diagnostics(
        shifted).rank_normalized_rhat > 1.1
    @test_throws ArgumentError Evaluation.split_rank_diagnostics(
        independent[:, 1:1])
end
