...
The NUTS row is AdvancedHMC-only and reports its observed average leapfrog
count because its work per transition is dynamic. The repository's checked
dynamic-tree samplers are not labeled or benchmarked as NUTS: equivalence with
a production recursive NUTS implementation is outside their current claim.
...
documentation page and SVG chart from those files.
